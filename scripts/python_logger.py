...
class StructuredLogger:
    ...
    def error(
        ...
        self._log_with_context(
            logging.ERROR, message, extra_fields, correlation_id, exc_info
        )
    ...
    def critical(
        ...
        self._log_with_context(
            logging.CRITICAL, message, extra_fields, correlation_id, exc_info
        )
    ...
    def log_performance(
        ...
        self._log_with_context(
            level, f"Performance metric: {operation}", perf_data, correlation_id
        )
    ...
    def log_business_event(
        ...
        business_data = {"event": event, "category": "business"}
        if details:
            business_data.update(details)

        self._log_with_context(
            logging.INFO, f"Business event: {event}", business_data, correlation_id
        )
...
class PerformanceMonitor:
    ...
    def __enter__(self) -> "PerformanceMonitor":
        ...
        self.logger.debug(
            f"Starting operation: {self.operation}", correlation_id=self.correlation_id
        )
        return self
        ...
        if self.start_time is not None:
            ...
            if exc_type is not None:
                ...
                self.logger.log_performance(
                    self.operation, duration, correlation_id=self.correlation_id
                )
...
def monitor_performance(
    operation: str, threshold: float = 1.0, correlation_id: Optional[str] = None
) -> PerformanceMonitor:
