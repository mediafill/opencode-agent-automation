...
class CacheEntry:
    ...
    def __init__(self, key: str, value: Any, ttl_seconds: Optional[int] = None,
                 created_at: Optional[datetime] = None, access_count: int = 0):
        self.key = key
        self.value = value
    ...
    def _estimate_size(self) -> int:
        ...
        try:
            if isinstance(self.value, (str, bytes)):
                ...
                return sum(len(json.dumps(item)) if isinstance(item, dict) else len(str(item)) for item in self.value)
            else:
...
class IntelligentCache:
    ...
    def __init__(self, max_memory_mb: int = 100, default_ttl_seconds: int = 300,
                 enable_persistence: bool = True, persistence_dir: Optional[str] = None):
        self.max_memory_bytes = max_memory_mb * 1024 * 1024
        self.default_ttl_seconds = default_ttl_seconds
        self.enable_persistence = enable_persistence
        self.persistence_dir = Path(persistence_dir) if persistence_dir else Path('.claude/cache')
    ...
    def set(self, key: str, value: Any, ttl_seconds: Optional[int] = None,
        ...
        with self.lock:
            ...
            while (self._current_memory_usage() + entry.size_bytes > self.max_memory_bytes
                   and self.cache):
    ...
    def get_or_set(self, key: str, func: Callable, ttl_seconds: Optional[int] = None,
                   cache_type: Optional[str] = None, force_refresh: bool = False) -> Any:
    ...
    def _save_persistent_cache(self):
        ...
        try:
            ...
            with open(cache_file, 'w') as f:
                json.dump({'entries': entries_data, 'saved_at': datetime.now().isoformat()}, f, indent=2)
...
def log_cache_stats():
    ...
    logger.info(f"Cache Stats - Entries: {stats['entries']}, "
               f"Memory: {stats['memory_usage_mb']:.2f}MB/{stats['max_memory_mb']:.2f}MB, "
               f"Hit Rate: {stats['hit_rate']:.2%}")

# Periodic cache monitoring
def start_cache_monitoring(interval_seconds: int = 300):
