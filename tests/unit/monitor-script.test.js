...
describe('Monitor Script Tests', () => {
  ...
  describe('Status Command', () => {
    ...
    test('status command handles missing log directory', async () => {
      const { stdout } = await execAsync(`LOG_DIR="/nonexistent" bash "${scriptPath}" status`);
    ...
    test('status command analyzes task file when present', async () => {
      ...
      const { stdout } = await execAsync(`CLAUDE_DIR="${testClaudeDir}" bash "${scriptPath}" status`);
  ...
  describe('Log Analysis', () => {
    test('analyzes completed log files correctly', async () => {
      ...
      fs.writeFileSync(logFile, 'Starting task...\nProcessing data...\nTask completed successfully\n');

      const { stdout } = await execAsync(`LOG_DIR="${testLogDir}" bash "${scriptPath}" status`);
  ...
  describe('Summary Command', () => {
    ...
    test('summary counts different log types correctly', async () => {
      ...
      fs.writeFileSync(path.join(testLogDir, 'completed1.log'), 'Task completed successfully');
      fs.writeFileSync(path.join(testLogDir, 'completed2.log'), 'Another task completed successfully');
      fs.writeFileSync(path.join(testLogDir, 'failed1.log'), 'Error: Task failed');
      fs.writeFileSync(path.join(testLogDir, 'running1.log'), 'Processing...');

      const { stdout } = await execAsync(`LOG_DIR="${testLogDir}" bash "${scriptPath}" summary`);
    ...
    test('summary shows errors when present', async () => {
      fs.writeFileSync(path.join(testLogDir, 'error.log'), 'Error: Database connection failed\nFailed to execute query');
  ...
  describe('Process Detection', () => {
    test('get_agent_status function detects opencode processes', async () => {
      ...
      const { stdout } = await execAsync(`bash -c 'source "${scriptPath}"; get_agent_status'`);
  ...
  describe('Environment Configuration', () => {
    test('script uses environment variables correctly', async () => {
      ...
      const { stdout } = await execAsync(`PROJECT_DIR="${customDir}" bash "${scriptPath}" status`);
      expect(stdout).toContain('OpenCode Agent Status');
    });
  ...
  describe('Error Handling', () => {
    test('script handles permission errors gracefully', async () => {
      ...
      try {
        const { stdout } = await execAsync(`LOG_DIR="${restrictedDir}" bash "${scriptPath}" status`);
        expect(stdout).toContain('OpenCode Agent Status');
      } finally {
        // Clean up
  ...
  describe('Integration Tests', () => {
    test('full workflow with tasks and logs', async () => {
      ...
      const testTasks = {
        tasks: [
          { id: '1', status: 'completed', priority: 'high', description: 'Security audit' },
          { id: '2', status: 'in_progress', priority: 'medium', description: 'Add tests' },
          { id: '3', status: 'pending', priority: 'low', description: 'Documentation' },
          { id: '4', status: 'blocked', priority: 'high', description: 'API integration' }
        ]
      };
      fs.writeFileSync(tasksFile, JSON.stringify(testTasks, null, 2));
