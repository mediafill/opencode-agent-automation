...
describe("Enhanced Filtering Functionality Tests", () => {
  ...
  describe("Date Range Filtering", () => {
    ...
    test("checkDateRange filters by from date only", () => {
      ...
      expect(checkDateRange(new Date("2023-01-05"), fromDate, null)).toBe(
        false,
      );
    ...
    test("checkDateRange filters by date range", () => {
      ...
      expect(checkDateRange(new Date("2023-01-15"), fromDate, toDate)).toBe(
        true,
      );
      expect(checkDateRange(new Date("2023-01-05"), fromDate, toDate)).toBe(
        false,
      );
      expect(checkDateRange(new Date("2023-01-25"), fromDate, toDate)).toBe(
        false,
      );
      expect(checkDateRange(new Date("2023-01-10"), fromDate, toDate)).toBe(
        true,
      );
      expect(checkDateRange(new Date("2023-01-20"), fromDate, toDate)).toBe(
        true,
      );
    });
  ...
  describe("CSV Export Functionality", () => {
    ...
    test("exportFilteredAgents returns CSV data", () => {
      ...
      expect(csvData).toContain(
        "testing_agent_1,testing,completed,medium,100%",
      );
  ...
  describe("Performance and Edge Cases", () => {
    ...
    test("sortAgents handles missing properties", () => {
      ...
      expect(() =>
        sortAgents(agentsWithMissingProps, "priority-high"),
      ).not.toThrow();
      expect(() => sortAgents(agentsWithMissingProps, "newest")).not.toThrow();
      expect(() => sortAgents(agentsWithMissingProps, "status")).not.toThrow();
      expect(() =>
        sortAgents(agentsWithMissingProps, "progress"),
      ).not.toThrow();
    });

    test("checkDateRange handles invalid date formats", () => {
      expect(checkDateRange("invalid date", "2023-01-10", "2023-01-20")).toBe(
        false,
      );
      expect(
        checkDateRange(new Date("2023-01-15"), "invalid", "2023-01-20"),
      ).toBe(true);
      expect(
        checkDateRange(new Date("2023-01-15"), "2023-01-10", "invalid"),
      ).toBe(true);
    });
