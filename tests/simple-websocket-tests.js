...
try {
    ...
    global.Blob = function(data, options) { this.data = data; this.options = options; };
    global.setTimeout = (fn) => { try { fn(); } catch(e) { console.error('Timer error:', e); } };
    global.setInterval = () => 'interval_id';
    global.clearTimeout = () => {};
    global.clearInterval = () => {};
    
    // Mock Chart.js
    global.Chart = function() { return { update: () => {}, data: { datasets: [{ data: [] }] } }; };
    ...
    if (runTest('Demo Data Loading', () => {
        ...
        if (dashboardFunctions.agents.length === 0) throw new Error('No demo agents loaded');
        if (dashboardFunctions.tasks.length === 0) throw new Error('No demo tasks loaded');
        if (dashboardFunctions.logs.length === 0) throw new Error('No demo logs loaded');
    ...
    // Test 2: Agent Update
    total++;
    if (runTest('Agent Update Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const initialCount = dashboardFunctions.agents.length;
        const agentData = { id: 'test_agent_123', status: 'running', progress: 50, type: 'security', task: 'Testing' };
        
        dashboardFunctions.updateAgent(agentData);
        
        const agent = dashboardFunctions.agents.find(a => a.id === 'test_agent_123');
    ...
    // Test 3: Task Updates
    total++;
    if (runTest('Task Update Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const taskData = {
            id: 'test_task_456',
            type: 'testing',
        ...
        const task = dashboardFunctions.tasks.find(t => t.id === 'test_task_456');
    ...
    if (runTest('Log Entry Handling', () => {
        ...
        if (dashboardFunctions.logs.length !== initialCount + 1) throw new Error('Log not added');
        
        const log = dashboardFunctions.logs[dashboardFunctions.logs.length - 1];
        if (log.message !== 'Test log message') throw new Error('Log message incorrect');
    })) passed++;
    
    // Test 5: Message Handling
    total++;
    if (runTest('WebSocket Message Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const initialAgentCount = dashboardFunctions.agents.length;
        const testMessage = {
            type: 'agent_update',
            agent: { id: 'ws_test_agent', status: 'running', progress: 75, type: 'security', task: 'WebSocket test' }
        };
        
        dashboardFunctions.handleWebSocketMessage(testMessage);
        
        const agent = dashboardFunctions.agents.find(a => a.id === 'ws_test_agent');
    ...
    total++;
    if (runTest('Invalid Message Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const invalidMessages = [null, undefined, 'invalid', 123, {}, { type: 'unknown' }];
        
        invalidMessages.forEach((msg, i) => {
            try {
                dashboardFunctions.handleWebSocketMessage(msg);
            } catch (error) {
                throw new Error(`Should not throw on invalid message ${i}: ${error.message}`);
            }
        });
    })) passed++;
    ...
    total++;
    if (runTest('Connection Status Updates', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const mockElement = { className: '', textContent: '' };
        global.document.getElementById = () => mockElement;
        
        dashboardFunctions.updateConnectionStatus('connected', 'Connected successfully');
        
        if (mockElement.className !== 'connection-dot connected') throw new Error('Class not updated correctly');
        if (mockElement.textContent !== 'Connected successfully') throw new Error('Text not updated correctly');
    })) passed++;
    
    // Test 8: Full Status Message
    total++;
    if (runTest('Full Status Message Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const statusMessage = {
            type: 'full_status',
            agents: [
        ...
        if (dashboardFunctions.agents.length < 2) throw new Error(`Agents not updated from full status: got ${dashboardFunctions.agents.length}`);
        if (dashboardFunctions.tasks.length < 2) throw new Error(`Tasks not updated from full status: got ${dashboardFunctions.tasks.length}`);
        
        const agent = dashboardFunctions.agents.find(a => a.id === 'status_agent1');
        if (!agent) throw new Error('Status agent not found');
    })) passed++;
    
    // Test 9: System Alert Handling
    total++;
    if (runTest('System Alert Handling', () => {
        const dashboardFunctions = require('./dashboard-functions');
        ...
        if (dashboardFunctions.logs.length !== initialLogCount + 1) throw new Error('Alert log not added');
        
        const lastLog = dashboardFunctions.logs[dashboardFunctions.logs.length - 1];
        if (lastLog.level !== 'warn') throw new Error('Alert log level incorrect');
        if (!lastLog.message.includes('Test system alert')) throw new Error('Alert message not found in log');
    })) passed++;
    
    // Test 10: Log Limits
    total++;
    if (runTest('Log Entry Limits (1000 max)', () => {
        const dashboardFunctions = require('./dashboard-functions');
        const initialCount = dashboardFunctions.logs.length;
        ...
        if (dashboardFunctions.logs.length > 1000) {
            throw new Error(`Too many logs maintained: ${dashboardFunctions.logs.length}, should be max 1000`);
        }
    })) passed++;
