...
// Additional caching for expensive operations
let cachedFilteredAgents = null;
let cachedSearchResults = null;
let cachedChartData = null;
let lastFilterUpdate = 0;
let lastSearchUpdate = 0;
let lastChartUpdate = 0;
const FILTER_CACHE_TTL = 2000; // 2 seconds
const SEARCH_CACHE_TTL = 1000; // 1 second
const CHART_CACHE_TTL = 3000; // 3 seconds
  ...
  try {
    ...
    websocket.onopen = function () {
      ...
      connectionMetrics.reconnectCount =
        connectionAttempts > 0 ? connectionMetrics.reconnectCount + 1 : 0;
  ...
  heartbeatInterval = setInterval(() => {
    if (websocket && websocket.readyState === 1) {
      ...
      const timeSinceLastHeartbeat = connectionMetrics.lastHeartbeat
        ? Date.now() - connectionMetrics.lastHeartbeat.getTime()
        : Infinity;
...
function loadDemoData() {
  ...
  logs.push(
    ...
    },
    {
      ...
      level: "error",
      message: "Documentation agent failed to parse OpenAPI spec",
      agent: "docs_agent",
    },
    {
      time: new Date(Date.now() - 120000),
      level: "info",
      message: "Performance agent queued",
      agent: "perf_agent",
    },
  );
...
function updateAgent(agentData) {
  ...
  agentsIndex.set(
    agentData.id,
    agents[existingIndex >= 0 ? existingIndex : agents.length - 1],
  );
...
function updateTask(taskData) {
  ...
  tasksIndex.set(
    taskData.id,
    tasks[existingIndex >= 0 ? existingIndex : tasks.length - 1],
  );
  ...
  if (!performanceData) {
    performanceData = {
      connectionUptime: connectionMetrics.connectTime
        ? Date.now() - connectionMetrics.connectTime.getTime()
        : 0,
...
function updateTaskDistributionChart() {
  ...
  // Check if we can use cached chart data
  if (cachedChartData && now - lastChartUpdate < CHART_CACHE_TTL) {
    charts.taskDistribution.data.datasets[0].data = cachedChartData;
...
// Filter functions - optimized with indexes and caching
function filterAgents() {
  ...
  const cacheKey = `${statusFilter}|${typeFilter}|${searchFilter}`;
  const now = Date.now();

  // Check if we can use cached results
  if (
    cachedFilteredAgents &&
    cachedFilteredAgents.key === cacheKey &&
    now - lastFilterUpdate < FILTER_CACHE_TTL
  ) {
    renderFilteredAgents(cachedFilteredAgents.results);
    return;
  }
  ...
  if (statusFilter && typeFilter) {
    ...
    filteredAgentIds = new Set(
      [...statusAgents].filter((id) => typeAgents.has(id)),
    );
  ...
  const filteredAgents = Array.from(filteredAgentIds)
...
// Separate rendering function to allow cached results to be rendered
function renderFilteredAgents(filteredAgents) {
  ...
  const agentCards = filteredAgents
    .map(
      (agent) => `
        <div class="card agent-card" onclick="showAgentDetails('${agent.id}')">
            <div class="agent-header">
...
function fuzzyMatch(searchTerm, searchFields) {
  return fuzzyMatchCached(searchTerm, searchFields);
}

function fuzzyMatchCached(searchTerm, searchFields) {
  const cacheKey = `${searchTerm}|${searchFields.join("|")}`;
  const now = Date.now();

  // Check cache first
  if (
    cachedSearchResults &&
    cachedSearchResults.key === cacheKey &&
    now - lastSearchUpdate < SEARCH_CACHE_TTL
  ) {
    return cachedSearchResults.result;
  }
...
function checkDateRange(agentDate, fromDate, toDate) {
  ...
  if (toDateObj && agentDateObj > new Date(toDateObj.getTime() + 86400000))
    return false; // Add one day for inclusive range
    ...
    filteredAgentIds.forEach((agentId) => {
      ...
      if (agent) {
        const matchesPriority =
          !priorityFilter || agent.priority === priorityFilter;
        const matchesSearch =
          !searchFilter ||
          fuzzyMatch(searchFilter.toLowerCase(), [
            agent.id,
            agent.task,
            agent.error || "",
            agent.type,
