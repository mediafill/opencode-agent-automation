...
describe('Vector Database Operations Integration', () => {
  ...
  describe('Complete Task History Workflow', () => {
    test('should handle complete task history lifecycle', async () => {
      ...
      const taskHistory = {
        ...
        decisions: [
          ...
          },
          {
            ...
            reasoning: 'Database operations can fail and need proper error recovery',
        ...
        outcome: 'Authentication service successfully refactored with improved performance and maintainability',
        learnings: [
      ...
      vectorDb.storeTaskHistory.mockResolvedValue('task_workflow_task_001_1234567890');
      ...
      const taskHistories = [
        {
          ...
          status: 'completed',
          learnings: ['Use try/catch in async functions', 'Log errors properly']
        },
        {
          ...
          status: 'completed',
          learnings: ['Input validation should be centralized', 'Use schema validation']
        },
        {
          ...
          status: 'completed',
          learnings: ['Handle network timeouts gracefully', 'Implement circuit breaker pattern']
        }
      ];
  ...
  describe('Agent Learning Accumulation', () => {
    test('should accumulate and retrieve agent learnings over time', async () => {
      ...
      const databaseLearnings = await vectorDb.getLearnings({ category: 'database' });
      ...
      expect(highImportanceLearnings.every(l => l.importance === 'high')).toBe(true);
    });
    ...
    test('should use learnings to inform decision making', async () => {
      ...
      vectorDb.querySimilarSolutions.mockResolvedValue([
        {
          id: 'learning_db_1',
          document: 'Database connection pooling prevents connection exhaustion...',
  ...
  describe('Cross-Task Knowledge Transfer', () => {
    test('should transfer knowledge between similar tasks', async () => {
      ...
      const apiRefactoringTask = {
        ...
        type: 'refactoring',
        description: 'Refactor REST API endpoints to use consistent error responses',
        status: 'completed',
        decisions: [
          {
      ...
      vectorDb.storeTaskHistory.mockResolvedValue('task_api_refactor_success_1');
      ...
      const similarTasks = [
        {
          ...
          document: 'Refactor REST API endpoints to use consistent error responses...',
          metadata: {
            taskId: 'api_refactor_success',
            taskType: 'refactoring',
            status: 'completed'
      ...
      const fullTaskHistory = await vectorDb.getTaskHistory('api_refactor_success');

      expect(fullTaskHistory).toBeDefined();
      expect(fullTaskHistory.learnings).toContain('Consistent error formats improve API usability');
  ...
  describe('Performance and Scalability', () => {
    test('should handle large volumes of task histories efficiently', async () => {
      ...
      const storePromises = tasks.map(task => vectorDb.storeTaskHistory(task));
    ...
    test('should handle concurrent queries efficiently', async () => {
      ...
      const testTasks = [
        { taskId: 'concurrent_1', type: 'testing', description: 'Concurrent test 1', status: 'completed' },
        { taskId: 'concurrent_2', type: 'testing', description: 'Concurrent test 2', status: 'completed' },
        { taskId: 'concurrent_3', type: 'deployment', description: 'Concurrent deployment', status: 'completed' }
      ];
      ...
      vectorDb.querySimilarSolutions
        .mockResolvedValueOnce([{ id: 'task_concurrent_1', metadata: { taskType: 'testing' } }])
        .mockResolvedValueOnce([{ id: 'task_concurrent_3', metadata: { taskType: 'deployment' } }]);
  ...
  describe('Error Recovery and Resilience', () => {
    test('should handle partial failures gracefully', async () => {
      ...
      const tasks = [
        { taskId: 'fail_1', type: 'testing', description: 'Will fail', status: 'pending' },
        { taskId: 'success_1', type: 'testing', description: 'Will succeed', status: 'completed' },
        { taskId: 'fail_2', type: 'testing', description: 'Will fail again', status: 'pending' },
        { taskId: 'success_2', type: 'testing', description: 'Will succeed', status: 'completed' }
      ];
    ...
    test('should maintain data consistency during failures', async () => {
      ...
      vectorDb.querySimilarSolutions.mockRejectedValueOnce(new Error('Query service unavailable'));
      ...
      await expect(vectorDb.querySimilarSolutions('test query')).rejects.toThrow('Query service unavailable');
    });
  });
  ...
  describe('Database Statistics and Monitoring', () => {
    test('should provide comprehensive database statistics', async () => {
      ...
      const tasks = [
        { taskId: 'stats_task_1', type: 'testing', description: 'Test 1', status: 'completed' },
        { taskId: 'stats_task_2', type: 'refactoring', description: 'Refactor 1', status: 'completed' }
      ];
