...
describe('Data Consistency Integration Tests', () => {
  ...
  const testProjectDir = path.join(__dirname, '..', 'fixtures', 'consistency-test-project');
  ...
  describe('Task and Status Data Synchronization', () => {
    ...
    test('synchronizes status updates across files', async () => {
      ...
      expect(updatedTasks[0].status).toBe(updatedStatus.consistency_task_1.status);
    });
  ...
  describe('WebSocket Broadcast Consistency', () => {
    test('WebSocket status updates reflect database state', async () => {
      ...
      const wsTask1 = data.data.tasks.find(t => t.id === 'consistency_task_1');
      const wsTask2 = data.data.tasks.find(t => t.id === 'consistency_task_2');
    ...
    test('real-time updates maintain consistency', async () => {
      ...
      const dbTasks = JSON.parse(await fs.readFile(tasksFile, 'utf8'));
      const addedTask = dbTasks.find(t => t.id === 'realtime_consistency_task');
      ...
      const wsTask = statusData.data.tasks.find(t => t.id === 'realtime_consistency_task');
  ...
  describe('Cross-Component Data Flow', () => {
    test('task manager updates are reflected in WebSocket broadcasts', async () => {
      ...
      const taskManagerProcess = spawn('python3', [
        taskManagerScript,
        '--test-mode',
        '--project-dir', testProjectDir,
        '--single-task', 'consistency_task_1'
      ], {
        stdio: 'pipe'
      });
      ...
      const monitorUpdates = async () => {
        try {
          ...
          if (data.type === 'task_status_update' || data.type === 'task_progress_update') {
    ...
    test('log file changes trigger consistent broadcasts', async () => {
      ...
      while (!logReceived && (Date.now() - startTime) < maxWait) {
        try {
          ...
          if (data.type === 'log_entry' && data.log.message.includes('Consistency test log entry')) {
            logReceived = true;
  ...
  describe('Error Handling Consistency', () => {
    ...
    test('handles concurrent data access without corruption', async () => {
      ...
      results.forEach(result => {
        ...
        expect(result.some(task => task.id === 'consistency_task_1')).toBe(true);
        expect(result.some(task => task.id === 'consistency_task_2')).toBe(true);
      });
    });
  });
});