...
describe('Vector Database Initialization and Configuration', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should handle ChromaDB connection failure', async () => {
      mockChromaClient.heartbeat.mockRejectedValue(new Error('Connection failed'));
      ...
      await expect(vectorDb.initialize()).rejects.toThrow('Failed to connect to ChromaDB');
      expect(vectorDb.isInitialized()).toBe(false);
    });

    test('should handle collection creation failure', async () => {
      mockChromaClient.createCollection.mockRejectedValue(new Error('Collection creation failed'));
      ...
      await expect(vectorDb.initialize()).rejects.toThrow('Failed to create collection');
      expect(vectorDb.isInitialized()).toBe(false);
    });
    ...
    test('should fail after max retries exceeded', async () => {
      mockChromaClient.createCollection.mockRejectedValue(new Error('Persistent failure'));
  ...
  describe('Configuration Validation', () => {
    test('should validate required configuration parameters', () => {
      ...
      expect(() => new VectorDatabase({ projectDir: testProjectDir })).toThrow('chromaUrl is required');
      expect(() => new VectorDatabase({
        projectDir: testProjectDir,
        chromaUrl: 'http://localhost:8000'
    ...
    test('should validate embedding model configuration', () => {
      const validModels = ['all-MiniLM-L6-v2', 'text-embedding-ada-002', 'sentence-transformers'];
