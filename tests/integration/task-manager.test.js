  ...
  describe("Task Lifecycle Management", () => {
    test("creates and processes tasks through complete lifecycle", async () => {
      ...
      try {
        ...
        taskManagerProcess = spawn(
          "python3",
          [taskManagerScript, "--test-mode", "--project-dir", testProjectDir],
          {
            stdio: "pipe",
          },
        );
        ...
        try {
          ...
          Object.entries(status).forEach(([taskId, taskStatus]) => {
            if (
              ...
              !processingOrder.some(
                (item) => item.taskId === taskId && item.status === "running",
              )
            ) {
  ...
  describe("Concurrent Task Processing", () => {
    test("processes multiple tasks concurrently when configured", async () => {
      ...
      const monitorConcurrency = async () => {
        try {
          const status = JSON.parse(await fs.readFile(taskStatusFile, "utf8"));
          const currentlyRunning = Object.keys(status).filter(
            (taskId) => status[taskId].status === "running",
          );

          maxConcurrent.value = Math.max(
            maxConcurrent.value,
            currentlyRunning.length,
          );
  ...
  describe("Task Progress Tracking", () => {
    ...
    test("provides meaningful progress steps", async () => {
      ...
      try {
        ...
        expect(
          stepUpdates.some(
            (step) => typeof step === "string" && step.length > 5,
          ),
        ).toBe(true);
      } finally {
        clearInterval(stepInterval);
  ...
  describe("Error Handling and Recovery", () => {
    test("handles corrupted task files gracefully", async () => {
      ...
      expect(errorOutput).toContain("JSON") ||
        expect(errorOutput).toContain("parse");
    }, 8000);
    ...
    test("recovers from system interruptions", async () => {
      ...
      const statusAfterInterrupt = JSON.parse(
        await fs.readFile(taskStatusFile, "utf8"),
      );
      ...
      expect(["completed", "failed", "cancelled", "running"]).toContain(
        finalStatus.interrupted_task.status,
      );
    }, 12000);
  });
});
