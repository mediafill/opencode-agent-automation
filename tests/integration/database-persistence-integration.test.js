...
describe("Database Persistence and Recovery Integration Tests", () => {
  ...
  describe("Database Persistence Across Operations", () => {
    ...
    test("should handle large datasets without corruption", async () => {
      ...
      expect(lastTask.metadata.complex_data.nested.object.a).toBe(
        largeTaskCount - 1,
      );
  ...
  describe("Database Backup and Recovery", () => {
    test("should create and restore from backups correctly", async () => {
      ...
      const tasksBackup = path.join(
        backupsDir,
        `tasks_backup_${backupTimestamp}.json`,
      );
      const statusBackup = path.join(
        backupsDir,
        `task_status_backup_${backupTimestamp}.json`,
      );
    ...
    test("should handle point-in-time recovery", async () => {
      ...
      const afterCreateStatus = JSON.parse(
        await fs.readFile(taskStatusFile, "utf8"),
      );
      timeline.push({
        timestamp: new Date().toISOString(),
      ...
      await fs.writeFile(
        tasksFile,
        JSON.stringify(recoveryPoint.tasks, null, 2),
      );
      await fs.writeFile(
        taskStatusFile,
        JSON.stringify(recoveryPoint.status, null, 2),
      );
  ...
  describe("Database Performance and Optimization", () => {
    ...
    test("should maintain performance with large datasets", async () => {
      ...
      const highPriorityTasks = readTasks.filter(
