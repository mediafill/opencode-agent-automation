...
describe('Enhanced Database Integration Tests', () => {
  ...
  beforeAll(async () => {
    testProjectDir = path.join(__dirname, 'fixtures', 'enhanced-db-integration-test');
  ...
  describe('Database Migration and Schema Evolution', () => {
    ...
    test('migrates data between schema versions', async () => {
      ...
      const migratedTasks = rawTasks.map(task => ({
        ...
        created_at: task.timestamp || task.created_at || new Date().toISOString(),
  ...
  describe('Database Encryption and Security', () => {
    ...
    test('handles secure credential storage', async () => {
      ...
      const storedCredentials = JSON.parse(await fs.readFile(secureFile, 'utf8'));
  ...
  describe('Database Indexing and Query Performance', () => {
    test('maintains indexes for efficient queries', async () => {
      ...
      const largeDataset = Array.from({ length: 200 }, (_, i) => ({
        ...
        files_pattern: `**/*${i + 1}.*`,
        created_at: new Date(Date.now() - Math.random() * 86400000).toISOString(), // Random date within last 24h
    ...
    test('handles complex queries with aggregations', async () => {
      ...
      complexDataset.forEach(task => {
        // Count by type
        aggregations.by_type[task.type] = (aggregations.by_type[task.type] || 0) + 1;

        // Count by status
        aggregations.by_status[task.status] = (aggregations.by_status[task.status] || 0) + 1;

        // Count by priority
        aggregations.by_priority[task.priority] = (aggregations.by_priority[task.priority] || 0) + 1;
        ...
        task.metadata.tags.forEach(tag => {
          aggregations.tag_popularity[tag] = (aggregations.tag_popularity[tag] || 0) + 1;
        });
      });

      // Calculate averages and rates
      aggregations.average_duration /= aggregations.total_tasks;
      aggregations.completion_rate = (aggregations.by_status.completed || 0) / aggregations.total_tasks;
      ...
      expect(Object.keys(aggregations.tag_popularity).length).toBeGreaterThan(0);
    });
  });
    ...
    test('handles connection timeouts and recovery', async () => {
      ...
      class TimeoutConnection {
        ...
        async readFile(path) {
          ...
          if (Math.random() < 0.2) {
            await new Promise(resolve => setTimeout(resolve, this.timeoutMs + 100));
