...
describe('Vector Database Task History Storage', () => {
  ...
  beforeEach(async () => {
    ...
    testProjectDir = path.join(__dirname, '..', 'fixtures', 'task-history-test');
  ...
  describe('Task History Storage', () => {
    ...
    test('should store failed task execution history', async () => {
      const failedTaskHistory = {
        ...
        error: 'Circular dependency detected between UserService and AuthService',
    ...
    test('should validate task history data structure', async () => {
      ...
      await expect(vectorDb.storeTaskHistory(invalidTaskHistory))
    ...
    test('should handle storage errors gracefully', async () => {
      ...
      vectorDb.storeTaskHistory.mockRejectedValue(new Error('Storage backend error'));

      await expect(vectorDb.storeTaskHistory(taskHistory))
        .rejects.toThrow('Storage backend error');
    });
  });
  ...
  describe('Task History Metadata', () => {
    ...
    test('should handle large metadata objects', async () => {
      ...
      expect(callArgs.metadatas[0].codeMetrics).toHaveProperty('linesOfCode', 15420);
    });
  });
});