...
describe('Enhanced Task Management Integration Tests', () => {
  ...
  beforeAll(async () => {
    testProjectDir = path.join(__dirname, 'fixtures', 'enhanced-task-management-test');
  ...
  describe('Task Dependencies and DAG Management', () => {
    ...
    test('handles circular dependencies gracefully', async () => {
      ...
      const resolvedTasks = circularTasks.map(task => ({
        ...
        status: task.metadata.dependencies.some(dep =>
          circularTasks.find(t => t.id === dep)?.metadata.dependencies.includes(task.id)
        ) ? 'blocked' : task.status
      }));
    ...
    test('manages complex dependency chains', async () => {
      ...
      function getExecutableTasks(completedTasks) {
        ...
        return fullComplexTasks.filter(task =>
    ...
    test('manages task execution time limits', async () => {
      ...
      function checkTimeouts() {
        ...
        tasksWithTimeouts.forEach(task => {
          if (task.status === 'running' && task.started_at && task.timeout_seconds) {
    ...
    test('handles recurring and periodic tasks', async () => {
      ...
      function getDueRecurringTasks() {
        ...
        return recurringTasks.filter(task => {
          if (!task.schedule || task.schedule.type !== 'recurring') return false;
  ...
  describe('Task Resource Management and Allocation', () => {
    test('manages resource allocation across tasks', async () => {
      ...
      function allocateResources(availableResources, task) {
        ...
        if (availableResources.cpu_cores >= requirements.cpu_cores &&
          ...
          return {
            ...
            disk_space_gb: availableResources.disk_space_gb - requirements.disk_space_gb
          };
        }
      ...
      function getSchedulableTasks() {
        ...
        const runningTasks = resourceTasks.filter(t => t.status === 'running');
        ...
        return resourceTasks
          ...
          .filter(task => allocateResources(availableResources, task) !== null)
          .sort((a, b) => {
            ...
            const aTotalResources = a.resource_requirements.cpu_cores + a.resource_requirements.memory_gb;
            const bTotalResources = b.resource_requirements.cpu_cores + b.resource_requirements.memory_gb;
      ...
      const memoryIntensiveIndex = schedulableTasks.findIndex(t => t.id === 'memory_intensive_task');
      ...
      function resolveResourceConflicts() {
        ...
        pending.forEach(pendingTask => {
          if (pendingTask.metadata.can_preempt) {
            ...
            const preemptableTasks = running.filter(runningTask =>
              runningTask.metadata.can_be_preempted &&
              runningTask.resource_requirements.cpu_cores === pendingTask.resource_requirements.cpu_cores &&
              runningTask.resource_requirements.memory_gb === pendingTask.resource_requirements.memory_gb
            );
    ...
    test('monitors and reports resource utilization', async () => {
      ...
      function calculateEfficiencyScore(task) {
        ...
        const memoryEfficiency = Math.min(task.resource_usage.memory_mb / 4096, 1); // Normalize to 4GB max
      ...
      function generateAlerts(taskReports) {
        ...
        taskReports.forEach(report => {
          ...
          if (report.resource_usage.memory_mb > 3072) { // 3GB
            alerts.push({
    ...
    test('monitors task health and performance', async () => {
      ...
      function assessTaskHealth() {
        ...
        healthTasks.forEach(task => {
          ...
          // Check progress rate
          if (task.health_metrics.progress_rate === 0 && task.status === 'running') {
      ...
      const healthyAssessment = healthAssessments.find(a => a.task_id === 'healthy_task');
      const unhealthyAssessment = healthAssessments.find(a => a.task_id === 'unhealthy_task');
    ...
    test('generates alerts for task anomalies', async () => {
      const anomalyTasks = [
        {
          ...
          error_history: [
            { timestamp: new Date(Date.now() - 300000).toISOString(), error: 'Network error' },
            { timestamp: new Date(Date.now() - 240000).toISOString(), error: 'Timeout' },
            { timestamp: new Date(Date.now() - 180000).toISOString(), error: 'Connection failed' }
          ],
          metadata: {
            estimated_duration: 120
          }
      ...
      function generateAlerts() {
        ...
        anomalyTasks.forEach(task => {
          ...
          if (task.status === 'failed' && task.retry_count >= task.max_retries) {
            alerts.push({
              type: 'task_failure',
              severity: 'critical',
              task_id: task.id,
          ...
          if (task.status === 'running' && task.started_at) {
            ...
            const expectedProgress = (elapsedMinutes / (task.metadata.estimated_duration / 60)) * 100;

            if (task.progress < expectedProgress * 0.5) { // Less than 50% of expected progress
              alerts.push({
                type: 'task_delay',
                severity: 'warning',
                task_id: task.id,
    ...
    test('tracks task performance metrics over time', async () => {
      const performanceTasks = [
        {
          ...
          performance_history: [
            { timestamp: new Date(Date.now() - 240000).toISOString(), cpu_usage: 45, memory_usage: 512 },
            { timestamp: new Date(Date.now() - 180000).toISOString(), cpu_usage: 67, memory_usage: 768 },
            { timestamp: new Date(Date.now() - 120000).toISOString(), cpu_usage: 89, memory_usage: 1024 },
            { timestamp: new Date(Date.now() - 60000).toISOString(), cpu_usage: 78, memory_usage: 896 }
          ],
          metadata: {
            estimated_duration: 240,
            actual_duration: 300
      ...
      function analyzePerformance() {
        ...
        const analysis = {
          ...
          efficiency: task.metadata.estimated_duration / task.metadata.actual_duration,
          peak_cpu_usage: 0,
          average_cpu_usage: 0,
          peak_memory_usage: 0,
        ...
        const memoryUsages = task.performance_history.map(h => h.memory_usage);

        analysis.peak_cpu_usage = Math.max(...cpuUsages);
        analysis.average_cpu_usage = cpuUsages.reduce((a, b) => a + b, 0) / cpuUsages.length;
        analysis.peak_memory_usage = Math.max(...memoryUsages);
        analysis.average_memory_usage = memoryUsages.reduce((a, b) => a + b, 0) / memoryUsages.length;
      ...
      expect(['stable', 'increasing', 'decreasing']).toContain(analysis.performance_trend);
