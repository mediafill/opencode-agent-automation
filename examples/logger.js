...
const customFormat = winston.format.printf(({ timestamp, level, message, service, environment, correlationId, ...meta }) => {
  const logEntry = {
    timestamp,
...
const requestLogger = (req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const logData = {
      ...
      responseSize: res.get('content-length') || (res.body ? Buffer.byteLength(res.body) : 0),
      referrer: req.get('Referrer') || req.get('Referer')
    };

    if (res.statusCode >= 400) {
      logData.errorCategory = res.statusCode >= 500 ? 'server_error' : 'client_error';
    }
